...
# velocity = dataset(dataFileName, dataCMFileName, 'velocity', )
bed, smb, surface = None, None, None
...
def mouseMoved(e):
    ...
    if e.isExit() is False:
        ...
        if np.abs(x) <= 10018 and np.abs(y) <= 17946:
            ...
            if currentMap == 0:
                # print 'mm 30'
                t = 'x: ' + str(x) + '\ty: ' + str(y) + '\t v: ' + str(v[int(np.ceil(np.abs(y)))][int(np.floor(np.abs(x)))])
                if inBed:
